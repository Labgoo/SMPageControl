...
- (void)_initialize {
    ...
    _orientation = SMPageControlOrientationHorizontal;
    _changeCurrentPageOnTouches = YES;
...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect {
    ...
    switch (_orientation) {
        case SMPageControlOrientationHorizontal:
            xOffset = [self _leftOffset];
            yOffset = 0.0f;
            break;
            
        case SMPageControlOrientationVertical:
            xOffset = 0.0f;
            yOffset = [self _topOffset];
            break;
    }
    ...
    for (NSUInteger i = 0; i < _numberOfPages; i++) {
        ...
        switch (_orientation) {
            ...
            case SMPageControlOrientationVertical:
                if (image) {
                    xOffset = [self _leftOffsetForWidth:image.size.width rect:rect];
                    CGFloat centeredYOffset = yOffset + floorf((_measuredIndicatorHeight - image.size.height) / 2.0f);
                    [image drawAtPoint:CGPointMake(xOffset, centeredYOffset)];
                } else if (maskingImage) {
                    xOffset = [self _leftOffsetForWidth:maskSize.width rect:rect];
                    CGFloat centeredYOffset = yOffset + floorf((_measuredIndicatorHeight - maskSize.height) / 2.0f);
                    CGRect imageRect = CGRectMake(xOffset, centeredYOffset, maskSize.width, maskSize.height);
                    CGContextDrawImage(context, imageRect, maskingImage);
                } else {
                    xOffset = [self _leftOffsetForWidth:_indicatorDiameter rect:rect];
                    CGFloat centeredYOffset = yOffset + floorf((_measuredIndicatorHeight - _indicatorDiameter) / 2.0f);
                    CGContextFillEllipseInRect(context, CGRectMake(xOffset, centeredYOffset, _indicatorDiameter, _indicatorDiameter));
                }
                yOffset += _measuredIndicatorHeight + _indicatorMargin;
                break;
...
- (CGFloat)_topOffset {
    ...
    switch (_verticalAlignment) {
        case SMPageControlVerticalAlignmentMiddle:
            top = CGRectGetMidY(rect) - (size.height / 2.0f);
            break;
...
- (CGFloat)_leftOffsetForWidth:(CGFloat)width rect:(CGRect)rect {
    CGFloat left = 0.0f;
    switch (_alignment) {
        case SMPageControlAlignmentCenter:
            left = CGRectGetMidX(rect) - (width / 2.0f);
            break;
        case SMPageControlAlignmentRight:
            left = CGRectGetMaxX(rect) - width;
            break;
        default:
            break;
    }
...
- (CGSize)sizeForNumberOfPages:(NSInteger)pageCount {
    switch (_orientation) {
        ...
        case SMPageControlOrientationVertical: {
            CGFloat marginSpace = MAX(0, pageCount - 1) * _indicatorMargin;
            CGFloat indicatorSpace = pageCount * _measuredIndicatorHeight;
            CGSize size = CGSizeMake(_measuredIndicatorHeight, marginSpace + indicatorSpace);
            return size;
        }
...
- (CGRect)rectForPageIndicator:(NSInteger)pageIndex {
    ...
    switch (_orientation) {
        case SMPageControlOrientationHorizontal: {
            ...
            CGRect rect = CGRectMake(0, top + size.height - _measuredIndicatorHeight, _measuredIndicatorWidth, _measuredIndicatorWidth);
            return rect;
        }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    if (!_changeCurrentPageOnTouches) {
        return;
    }
    ...
    switch (_orientation) {
        case SMPageControlOrientationVertical: {
            CGFloat top = [self _topOffset];
            CGFloat middle = top + (size.height / 2.0f);
            if (point.y < middle) {
